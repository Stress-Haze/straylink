...
$post_items = [];
if ($my_posts) {
    while ($post = mysqli_fetch_assoc($my_posts)) {
        $post_items[] = $post;
    }
}
...
foreach ($request_items as $item) {
    if ($item['status'] === 'pending') $pending_requests++;
    if ($item['status'] === 'approved') $approved_requests++;
}

$first_name_parts = preg_split('/\s+/', trim($member['full_name']));
$first_name = $first_name_parts[0] ?? 'Friend';
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="container py-5">
    <section class="account-hero mb-4">
        <div class="row g-4 align-items-center">
            <div class="col-lg-8">
                <p class="section-kicker mb-2">My App</p>
                <h1 class="fw-bold mb-2">Welcome back, <?= htmlspecialchars($first_name) ?>.</h1>
                <p class="text-muted mb-4">This is your mobile-friendly community hub for rescue activity, adoption progress, profile updates, and contribution milestones.</p>
                <div class="account-quick-actions">
                    <a href="gallery.php" class="quick-action-card">
                        <i class="bi bi-search-heart"></i>
                        <span>Browse animals</span>
                    </a>
                    <a href="rescue.php" class="quick-action-card">
                        <i class="bi bi-geo-alt"></i>
                        <span>Report a stray</span>
                    </a>
                    <a href="blog.php" class="quick-action-card">
            ...
            <div class="col-lg-4">
                <div class="account-summary-card">
                    <div class="summary-stat">
                        <strong><?= count($request_items) ?></strong>
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body text-center">
                    ...
                    <div class="text-start small text-muted">
                        <p class="mb-1">Newcomer: <strong>0</strong> karma</p>
                        <p class="mb-1">Contributor: <strong>10</strong> karma</p>
                        <p class="mb-1">Trusted: <strong>25</strong> karma</p>
                        <p class="mb-0">Champion: <strong>50</strong> karma</p>
                    </div>
                    <hr>
                    <div class="text-start small text-muted">
                            ...
                            <?php foreach ($request_items as $r): ?>
                                <div class="mobile-request-card">
                                    <div class="d-flex justify-content-between align-items-start gap-3 mb-2">
                                        <div>
                                            <strong><?= htmlspecialchars($r['animal_name'] ?? 'Unnamed') ?></strong>
                                            <div class="small text-muted"><?= ucfirst($r['species']) ?></div>
                                        </div>
                                        <span class="badge bg-<?=
                                            $r['status'] === 'approved' ? 'success' :
                                            ($r['status'] === 'rejected' ? 'danger' :
                                    ...
                                    <div class="small text-muted mb-2">Shelter: <?= htmlspecialchars($r['shelter_name'] ?? '-') ?></div>
                                    <div class="d-flex justify-content-between align-items-center">
                                        <small class="text-muted"><?= date('M d, Y', strtotime($r['created_at'])) ?></small>
                                        <?php if ($r['contact_number']): ?>
                                            <a href="tel:<?= $r['contact_number'] ?>" class="btn btn-outline-success btn-sm">Call</a>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                ...
                <div class="card-body">
                    <form method="POST" enctype="multipart/form-data">
                        <div class="mb-3">
                            ...
                            <input type="text" name="title" class="form-control" required placeholder="e.g. Why I adopted a stray dog">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Content</label>
                            <textarea name="body" class="form-control" rows="6" required placeholder="Share your story, experience or awareness message..."></textarea>
                        </div>
                        <div class="mb-3">
