...
if (isset($_GET['approve']) && is_numeric($_GET['approve'])) {
    $id = (int)$_GET['approve'];
    mysqli_query($conn, "UPDATE adoption_requests SET status = 'approved' WHERE id = $id AND shelter_id = $shelter_id");
    // Mark animal as reserved
    $req = mysqli_fetch_assoc(mysqli_query($conn, "SELECT animal_id FROM adoption_requests WHERE id = $id"));
    if ($req) mysqli_query($conn, "UPDATE animals SET adoption_status = 'reserved' WHERE id = {$req['animal_id']}");
...
$requests = mysqli_query($conn, "
    SELECT ar.*, a.name AS animal_name, a.species, m.full_name, m.phone, m.email
...
<nav class="navbar navbar-expand-lg navbar-dark bg-success">
    <div class="container-fluid">
        <a class="navbar-brand fw-bold" href="index.php">🐾 <?= htmlspecialchars($shelter['shelter_name']) ?></a>
    ...
    <div class="row">
        ...
        <nav class="col-md-2 bg-light sidebar min-vh-100 p-3">
            <ul class="nav flex-column">
                ...
                <li class="nav-item">
                    <a class="nav-link active" href="requests.php"><i class="bi bi-envelope"></i> Adoption Requests</a>
                </li>
                <li class="nav-item">
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <?php while ($r = mysqli_fetch_assoc($requests)): ?>
                            <tr>
                                ...
                                <td>
                                    <small><?= htmlspecialchars(substr($r['message'] ?? '—', 0, 80)) ?></small>
                                </td>
